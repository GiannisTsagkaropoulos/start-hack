...
def decide(purchase_updates: dict | None = None):
    purchase = deepcopy(PURCHASE)
    purchase.update(purchase_updates or {})
    policy = mandate_snapshot_to_engine_policy({"hard_rules": RULES})
...
def test_required_order_term_failure_and_unknown():
    ...
    assert decide({"order_cancellable": "unknown"}).decision == "step_up"
